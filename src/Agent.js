...
`;
  ...
  /**
   * Search chat history using the tiered search system
   * Integrates Victor Cache (Tier 1) with Atlas Vector Search (Tier 2/3)
   * @param {string} query - Natural language search query
   * @param {Object} options - Search options
   * @param {string} [options.sessionId] - Specific session to search (defaults to current session)
   * @param {string} [options.userId] - User ID for Layer B cache access
   * @param {Object} [options.dateRange] - Date range filter { start, end }
   * @param {string} [options.roleFilter] - Filter by role (user, assistant, system, tool)
   * @param {string} [options.searchMode] - Override search mode (victor-code, sentinel-triage, general-search)
   * @param {number} [options.limit] - Maximum results to return (default: 5)
   * @param {boolean} [options.enableCache] - Enable Victor Cache (default: true)
   * @param {number} [options.minConfidence] - Minimum confidence for early termination (default: 0.9)
   * @returns {Promise<Object>} - Search results with tier information
   */
  async searchHistory(query, options = {}) {
    ...
    const searchOptions = {
      ...
      estimatedDatasetSize: options.estimatedDatasetSize || 10000,
      ...options
    };

    try {
      // Step 1: Update Victor Cache with current session messages
      if (searchOptions.enableCache) {
        // Get current session messages for Layer A
        const currentMessages = await this.loadHistory(searchOptions.sessionId);
        victorCache.updateLayerA(currentMessages);
        
        // If userId provided, also update Layer B with recent sessions
        if (searchOptions.userId) {
          // This would typically load recent sessions for the user
          // For now, we'll rely on the tiered search orchestrator to handle this
        }
      }

      // Step 2: Perform tiered search
      const searchResult = await tieredSearch.search(query, searchOptions);

      // Step 3: Record search metrics
      ...
      this.emit('search-completed', {
        ...
        layer: searchResult.layer || 'unknown',
        hit: searchResult.hit || false,
        confidence: searchResult.confidence || 0,
        resultCount: searchResult.results?.length || 0,
        searchMode: searchResult.searchMode || 'unknown'
      });

      // Log search result for debugging
      if (this.debug) {
        console.log(`🔍 [AGENT SEARCH] Query: "${query.substring(0, 30)}..."`);
        console.log(`   Tier: ${searchResult.tier}, Layer: ${searchResult.layer || 'N/A'}`);
        console.log(`   Confidence: ${searchResult.confidence?.toFixed(3) || 'N/A'}`);
      ...
      console.error(`❌ [AGENT SEARCH] Search failed for query: "${query}"`, error);
  ...
  /**
   * Get search statistics from Victor Cache and Tiered Search
   * @returns {Object} - Combined search statistics
   */
  getSearchStats() {
    const victorCacheStats = victorCache.getStats();
    const tieredSearchStats = tieredSearch.getMetrics();
    
    return {
      victorCache: victorCacheStats,
      tieredSearch: tieredSearchStats,
      agentSessionId: this.sessionId,
      timestamp: Date.now()
    };
  }
  ...
  /**
   * Clear search cache (both Victor Cache and Tiered Search)
   */
  clearSearchCache() {
    victorCache.clear();
    tieredSearch.clearCache();
    
    this.emit('search-cache-cleared', {
      timestamp: Date.now(),
      sessionId: this.sessionId
    });
    
    console.log(`🧹 [AGENT SEARCH] Cache cleared for session ${this.sessionId}`);
  }

  /**
   * Health check for search system
   * @returns {Promise<Object>} - Health status of search components
   */
  async checkSearchHealth() {
    try {
      const results = await Promise.all([
        victorCache.healthCheck(),
        tieredSearch.checkTieredSearchHealth()
      ]);

      const overallStatus = results.every(r => r.status === 'healthy') ? 'healthy' : 
                           results.some(r => r.status === 'error') ? 'error' : 'unhealthy';

      return {
        status: overallStatus,
        message: overallStatus === 'healthy' ? 'Search system is healthy' : 'Search system has issues',
        components: {
          victorCache: results[0],
          tieredSearch: results[1]
        },
        agentSessionId: this.sessionId,
        timestamp: Date.now()
      };
      ...
      return {
        status: 'error',
        message: `Search health check failed: ${error.message}`,
        components: {},
        agentSessionId: this.sessionId,
        timestamp: Date.now()
      };
  ...
  /**
   * Execute Victor mode with integrated search capabilities
   * @param {Array} history - Conversation history
   * @param {string} userInput - User input message
   * @param {Function} [onChunk] - Optional callback for each text token
   * @returns {Promise<{response: string, fullMessages: Array}>} - Agent's response and full conversation
   */
  async executeVictorWithSearch(history, userInput, onChunk) {
    // First, try to search for relevant context in the current conversation
    try {
      const searchResult = await this.searchHistory(userInput, {
        searchMode: 'victor-code',
        enableCache: true,
        minConfidence: 0.85 // Slightly lower threshold for Victor mode
      });

      if (searchResult.hit && searchResult.results.length > 0) {
        // Add search context to system prompt for Victor mode
        const searchContext = this._formatSearchContext(searchResult.results);
      ...
      console.warn('Search failed in Victor mode, proceeding without context:', error);
    }

    // Fall back to standard Victor execution
    return this.executeVictor(history, userInput, onChunk);
  }

  /**
   * Execute Sentinel mode with integrated search capabilities
   * @param {Array} history - Conversation history
   * @param {string} userInput - User input message
   * @param {string} [batchType] - Type of batch processing (e.g., 'triage')
   * @param {Function} [onChunk] - Optional callback for each text token
   * @returns {Promise<{response: string, fullMessages: Array}>} - Agent's response and full conversation
   */
  async executeSentinelWithSearch(history, userInput, batchType = 'triage', onChunk) {
    // For Sentinel mode, we want to search across multiple sessions
    try {
      const searchResult = await this.searchHistory(userInput, {
        searchMode: 'sentinel-triage',
        enableCache: true,
        minConfidence: 0.7, // Lower threshold for broader search
        userId: this.userId // This would need to be set in the agent config
      });

      if (searchResult.hit && searchResult.results.length > 0) {
        // Add search context to system prompt for Sentinel mode
        const searchContext = this._formatSearchContext(searchResult.results, true);
        const enhancedSystemPrompt = this.systemPrompt + `\n\n## HISTORICAL CONTEXT\n${searchContext}`;
      ...
      console.warn('Search failed in Sentinel mode, proceeding without context:', error);
    }

    // Fall back to standard Sentinel execution
    return this.executeBatch(history, userInput, batchType, onChunk);
  }

  /**
   * Format search results as context for the LLM
   * @param {Array} results - Search results
   * @param {boolean} [includeSessionInfo] - Include session information
   * @returns {string} - Formatted context string
   * @private
   */
  _formatSearchContext(results, includeSessionInfo = false) {
    if (!results || results.length === 0) {
      return "No relevant context found.";
    }

    const contextLines = ["Recent relevant messages:"];
    
    results.slice(0, 3).forEach((result, index) => {
      if (result.layer === 'layer-a' || result.source === 'current-session') {
        contextLines.push(`[${index + 1}] ${result.role}: ${result.content.substring(0, 200)}...`);
      } else if (result.layer === 'layer-b' || result.source === 'recent-sessions') {
        if (includeSessionInfo) {
          contextLines.push(`[${index + 1}] Session ${result.sessionId}: ${result.summary || result.topic}`);
        } else {
          contextLines.push(`[${index + 1}] Previous session: ${result.summary || result.topic}`);
        }
      }
    });
