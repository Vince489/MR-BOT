...
// node src/scripts/chat-6

const SYSTEM_PROMPT = fs.readFileSync('./src/docs/persona-4.md', 'utf8');

async function main() {
  console.log('🤖 AUTOBOT Chat Interface v6.0 - OPENROUTER MINIMAX MODEL');
  console.log('===========================================================================\n');
  ...
  const agent = new Agent({
    ...
    provider: 'openrouter',
    model: process.env.open || 'nousresearch/hermes-3-llama-3.1-405b:free',
    appName: 'MR-BOT',
  ...
  if (trimmed.startsWith('/')) {
    ...
    try {
      ...
      if (error.message.includes('API key')) {
        console.error('   Please check your OPENROUTER_API_KEY in .env file.');
      }
    }
